...
RICE v2 将 **PicoC** C 语言解释器移植到 **STM32H750VBTx**（Cortex-M7, 480 MHz），运行在 **FreeRTOS** 之上，提供基于多任务的交互式 C 脚本环境。相比 v1 的核心架构改进是任务级隔离：串口 I/O 和脚本执行作为独立的 FreeRTOS 任务运行，使得 `:abort` 能够响应式地中断死循环，无需看门狗复位。
...
- 运动控制任务（轨迹规划、步进脉冲生成）
- 监控任务（温度、限位、异常检测）
- 面向 PicoC 脚本的应用层 API（如 `motor_move_to()`）
...
- 任务优先级和栈大小为初始估算值，可能需要调优
- 未启用电源管理/睡眠模式

## 相关项目

- [RICE v1](https://github.com/mosking128/PicoScript) — 稳定裸机版本，推荐生产使用
